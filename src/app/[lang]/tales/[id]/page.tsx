...
import { estimateReadTime, getTale, getTales, getLanguages, toPaperMeta, buildBacklinks, getGlossary, getAlternateLanguages, matchesPerspectiveView } from '@/lib/content';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post = getTale(lang, id);
  if (!post) return { title: 'Not Found' };

  const fm = post.frontmatter;
  const postUrl = `https://fractalresonance.com/${lang}/tales/${fm.id}`;
  const alternates = getAlternateLanguages('tales', fm.id);
...
export default async function TalesPostPage({ params }: Props) {
  const { lang, id } = await params;
  const post = getTale(lang, id);
  if (!post) notFound();
  ...
  const staticTargets = new Set(['about', 'articles', 'papers', 'books', 'tales', 'formulas', 'positioning', 'mu-levels', 'graph', 'privacy', 'terms']);
  ...
  return (
    <>
      ...
      <PageShell
        leftMobile={<TalesSidebar lang={lang} currentId={id} basePath={basePath} view="kasra" variant="mobile" />}
        leftDesktop={<TalesSidebar lang={lang} currentId={id} basePath={basePath} view="kasra" />}
      ...
      >
        ...
        <nav className="text-sm text-frc-text-dim mb-8">
          ...
          <a href={`${basePath}/tales`} className="hover:text-frc-gold">Tales</a>
          <span className="mx-2">/</span>
