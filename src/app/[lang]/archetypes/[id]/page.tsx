...
import { schemaArchetypePage } from '@/lib/schema';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const topic = getArchetype(lang, id);
  if (!topic) return { title: 'Not Found' };

  const fm = topic.frontmatter;
  const author = fm.author || 'FRC';
  const url = `https://fractalresonance.com/${lang}/archetypes/${fm.id}`;
  const alternates = getAlternateLanguages('archetypes', fm.id);
...
export default async function TopicPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <PageShell
        leftMobile={<ArchetypesSidebar lang={lang} currentId={id} basePath={basePath} view="kasra" variant="mobile" />}
        leftDesktop={<ArchetypesSidebar lang={lang} currentId={id} basePath={basePath} view="kasra" />}
        right={<TableOfContents items={tocItems} />}
      >
        <nav className="text-sm text-frc-text-dim mb-8">
          <a href={basePath} className="hover:text-frc-gold">FRC</a>
          <span className="mx-2">/</span>
          <a href={`${basePath}/archetypes`} className="hover:text-frc-gold">Archetypes</a>
          <span className="mx-2">/</span>
