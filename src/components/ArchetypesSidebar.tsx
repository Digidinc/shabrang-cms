...
export function ArchetypesSidebar({ lang, currentId, basePath, view, variant = 'desktop' }: ArchetypesSidebarProps) {
      ...
      <details open={!isMobile}>
        {isMobile && (
          <summary className="px-4 py-3 text-sm text-frc-text cursor-pointer select-none">
            <span className="text-xs uppercase tracking-wider text-frc-steel">Browse archetypes</span>
          </summary>
        )}
        <nav className={isMobile ? 'py-3 px-4 text-sm' : 'py-6 px-4 text-sm sticky top-0'}>
          <div className="mb-4">
            <h3 className="text-xs uppercase tracking-wider text-frc-steel mb-2 px-2">Archetypes</h3>
            <ul className="space-y-0.5 max-h-[70vh] overflow-y-auto pr-1">
              {archetypes.map((t) => (
                <li key={t.frontmatter.id}>
                  <Link
                    href={`${base}/archetypes/${t.frontmatter.id}`}
